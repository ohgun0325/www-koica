...
class DataSets:
    ...
    def load_train(self, file_path: Optional[str] = None) -> pd.DataFrame:
        ...
        Args:
            file_path: 파일 경로 (기본값: koica/koicainternational.csv)
            
        Returns:
            훈련 데이터프레임
        """
        if file_path is None:
            # 기본 경로: koica/koicainternational.csv
            current_dir = Path(__file__).parent
            file_path = str(current_dir / "koicainternational.csv")
        
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"파일을 찾을 수 없습니다: {file_path}")
        
        self._train = pd.read_csv(file_path, encoding='utf-8-sig', skiprows=3)
        self._fname = os.path.basename(file_path)
        self._dname = os.path.dirname(file_path)
        
        # 빈 컬럼 및 행 제거
        self._train = self._train.dropna(axis=1, how='all')
        self._train = self._train.dropna(axis=0, how='all')
    ...
    def load_test(self, file_path: Optional[str] = None) -> pd.DataFrame:
        ...
        self._test = pd.read_csv(file_path, encoding='utf-8-sig', skiprows=3)
        
        # 빈 컬럼 및 행 제거
        self._test = self._test.dropna(axis=1, how='all')
        self._test = self._test.dropna(axis=0, how='all')
    ...
    def save_train(self, file_path: Optional[str] = None) -> None:
        ...
        if file_path is None:
            if self._sname:
                file_path = os.path.join(self._sname, "koicainternational.csv")
            else:
                raise ValueError("저장 경로를 지정해주세요.")
        
        self._train.to_csv(file_path, index=False, encoding='utf-8-sig')
    ...
    def save_test(self, file_path: Optional[str] = None) -> None:
        ...
        self._test.to_csv(file_path, index=False, encoding='utf-8-sig')
