...
class DataSets:
    ...
    def load_train(self, file_path: Optional[str] = None) -> pd.DataFrame:
        ...
        Args:
            file_path: 파일 경로 (기본값: titanic/train.csv)
            
        Returns:
            훈련 데이터프레임
        """
        if file_path is None:
            # 기본 경로: titanic/train.csv
            current_dir = Path(__file__).parent
            file_path = str(current_dir / "titanic" / "train.csv")
        
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"파일을 찾을 수 없습니다: {file_path}")
        
        self._train = pd.read_csv(file_path)
    ...
    def load_test(self, file_path: Optional[str] = None) -> pd.DataFrame:
        ...
        Args:
            file_path: 파일 경로 (기본값: titanic/test.csv)
            
        Returns:
            테스트 데이터프레임
        """
        if file_path is None:
            # 기본 경로: titanic/test.csv
            current_dir = Path(__file__).parent
            file_path = str(current_dir / "titanic" / "test.csv")
        
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"파일을 찾을 수 없습니다: {file_path}")
        
        self._test = pd.read_csv(file_path)
    ...
    def save_train(self, file_path: Optional[str] = None) -> None:
        ...
        if file_path is None:
            if self._sname:
                file_path = os.path.join(self._sname, "train.csv")
            else:
                raise ValueError("저장 경로를 지정해주세요.")
        
        self._train.to_csv(file_path, index=False)
    ...
    def save_test(self, file_path: Optional[str] = None) -> None:
        ...
        if file_path is None:
            if self._sname:
                file_path = os.path.join(self._sname, "test.csv")
            else:
                raise ValueError("저장 경로를 지정해주세요.")
        
        self._test.to_csv(file_path, index=False)
